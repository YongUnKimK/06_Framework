...
SELECT BOARD_NO, BOARD_CODE, BOARD_TITLE, MEMBER_NICKNAME
	FROM "BOARD"
...
-- 업로드 파일에서 회원번호가 로그인한회원번호랑 같은 파일 조회
...
START WITH PARENT_COMMENT_NO IS NULL --부모 댓글이 시작 지점
CONNECT BY PRIOR COMMENT_NO = PARENT_COMMENT_NO --연결
ORDER SIBLINGS BY COMMENT_NO; -- SIBLINGS : 형제 / 자매 (같은레벨) 끼리 번호순으로정렬
...
-- /myPage/profile/20241121093933_00001.jfif (파일명만조회)
...
SELECT CHATTING_ROOM_NO
	...
	,(SELECT COUNT(*) FROM MESSAGE M WHERE M.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO AND READ_FL = 'N' AND SENDER_NO != 5) NOT_READ_COUNT
	...
	WHERE OPEN_MEMBER = 5
OR PARTICIPANT = 5
ORDER BY MAX_MESSAGE_NO DESC NULLS LAST; -- 최근 메시지가 없는 경우를 고려(메시지가 없다면 null을 맨아래정렬)
