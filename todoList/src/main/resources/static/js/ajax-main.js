console.log("ajax-min.js loaded.");
...
/* 
  ...
  Promise 객체는 세 가지 상태를 가짐
   ...
   - Rejected ( 거절 됨 ) : 비동기 작업이 실패한 경우 

  - 콜백( callback function ) 함수는 특정 작업이나 이벤트가 완료된 후 호출되는 함수
...
function getTotalCount() { // 함수의 정의
  ...
  fetch("/ajax/totalCount") // 서버로 비동기 요청  ( 기본값 : GET 요청 )
...
function getCompleteCount() { // 함수 정의
  ...
  // 첫번째 then()의 response : 
  // - 응답 결과, 요청 주소, 응답 데이터 등이 담겨잇음

  // 두 번째 then()의 result : 
  // - 첫번째 then 에서 알맞게 데이터(text() , json() )가 변환 가 된 응답

  // response.text() : 응답 데이터를 텍스트 형태로 변환

  fetch("/ajax/completeCount") // 서버로 비동기 요청 ( 기본값 : GET 요청 )
...
//console.log(param);
  ...
  .then(result => { // 첫번쨰 then에서 반환된 값 중 변환된 text를 result 에 저장
...
const selectTodoList= () => {
  ...
  .then(todoList => {
    ...
    // - JS Object 타입을 stirng 형태의 JSON 데이터로 변환
    ...
    for(let todo of todoList) { // 향상된 for문 형태
...
changeComplete.addEventListener("click", () => {
  ...
  fetch("/ajax/changeComplete", {
    ...
    headers : {"Content-Type" : "application/json"},
    body : JSON.stringify(obj) // js Object 형태인 obj를 Json 형싱그로 변경
  })
  .then( resp => resp.text() )
  .then( result => {
    //console.log(result);
...
updateView.addEventListener("click", () => {
  ...
  updateContent.value = popupTodoContent.innerHTML.replaceAll("<br>", "/n");
...
// 수정 레이어에서 취소버튼 (#updateBtn) 이 클릭되었을 때

updateCancel.addEventListener("click", () => {

  // 현재 보이고 있는 수정 레이어 숨기고
  updateLayer.classList.add("popup-hidden");
...
// 수정 레이어 -> 수정 버튼 클릭 (#updateBtn) 시 

updateBtn.addEventListener("click", e => {
    
  // 서버로 전달해야되는 값을 객체로 묶어줌
  // html data-todo-no (접근)
  // js dataset.todoNo
  const obj = {
...
getCompleteCount(); // 함수의 호출
getTotalCount(); // 함수의 호출
selectTodoList(); // 함수 호출쓰