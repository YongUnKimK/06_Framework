...
const MAX_SIZE = 1024 * 1024 * 5;  // 최대 파일 크기 설정 (5MB) -> 바이트 단위 (5,242,880byte)

const defaultImageUrl = `${window.location.origin}/images/user.png`;
// 절대경로로 기본 이미지 URL 설정 ${}=> http://localhost
// -> http://localhost/images/user.png 

let statusCheck = -1; // -1이면 초기상태, 0 : 이미지 삭제, 1: 새 이미지 선택
let previousImage = profileImg.src; // 이전이미지 URL 경로 기록 (초기 상태의 URL 저장 )
...
imageInput.addEventListener("change", () => {
  // change 이벤트 : 기존에 있던 값과 다르다면,

  // console.log(imageInput.files); ( input 태그는 FileList로 저장.. )
