...
public class EditBoardController {
	...
	public String boardInsert(@PathVariable("boardCode") int boardCode,
		 ...
		 * 
		 * [문제점] !?
		 * - 파일이 선택되지 않은 input태그도 제출되고 있음,, (제출은 되어 있는데 데이터는 없음)
		 * 
		 * -> 파일 선택이 안된 input 태그 값을 서버에 저장하려고 하면 오류 발생....
		 * 
		 * [해결방법]
		 * - 서버에 바로 저장 x
		 * -> List<MultipartFIle>에 실제로 제출된 파일이 잇는지 확인하는 로직을 추가로 구성..! (서비스단넘어가서함다)
		 * 
	 ...
	 * @param loginMember		 : 현재 로그인한 회원이 작성한 글이 맞는지 검사하는 용도
	 ...
	 * @param deleteOrderList	: 삭제된 이미지 순서가 기록되어 있는 문짜열 ( 1, 2, 3)
