...
@Configuration // 서버 실행시 해당 클래스 내 모든 메서드가 실행됨
public class FilterConfig {
	
	@Bean // 반환된 객체를 Bean으로 등록 : LoginFilter로 타입을 제한한 객체를 Bean으로 등록한다.
	public FilterRegistrationBean<LoginFilter> loginFilter() {
		...
		String[] filteringURL = {"/myPage/*", "/editBoard/*", "/chatting/*"};  // 마이페이지, 게시글수정, 채팅요청 필터링
		// String[]을 List로 변환
		// Araays.asList(filteringURL) -> List (.setUrlPatterns은 컬렉션으로받아야함 )
