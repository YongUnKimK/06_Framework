...
/*
 * aop사용하려면 build.gradle에 추가해야하는코드있음!!
 * spring boot starter를 스프링버전이랑 맞추는게 좋음
...
public class TestAspect {
	...
	// .. : 0개 이상의 하위 패키지 나타냄
	// *Controller* : 이름이 "Controller"라는 문자열을 포함하는 모든 클래스를 대상으로 함
	// .* : 모든 메서드를 나타냄
	// (..) : 0개 이상의 파라미터를 나타냄
	
	@Before("execution(* edu.kh.project..*Controller*.*(..))")
	public void testAdvice() {
		log.info("------------testAdvice() 수행됨--------------- 오늘은금요일ㅋ");		
	}
	...
	public void controllerEnd(JoinPoint jp) {
		...
		String className = jp.getTarget().getClass().getSimpleName(); // MemberController 식으로나옴.
		
		// 실행된 컨트롤러 메서드 이름을 얻어오기
		String methodName = jp.getSignature().getName(); // 메서드 얻어오는 명령어 ex) mainPage()
