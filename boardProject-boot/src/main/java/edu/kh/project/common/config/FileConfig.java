...
import org.springframework.context.annotation.PropertySource;import org.springframework.util.unit.DataSize;
...
public class FileConfig implements WebMvcConfigurer {
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		// Spring MVC에서 정적 리소스 ( 이미지, CSS , JS 파일 )의 요청을 처리하기 위해 사용하는 클래스
		...
		// 클라이언트가 /myPage/file/** 패턴으로 이미지를 요청할 때에
	...
	public MultipartConfigElement configElement() {
		...
		// 아래 3개는 Long타입이기 때문에 byte로 바꿔서 반환
		
		factory.setFileSizeThreshold(DataSize.ofBytes(fileSizeThreshold));
		factory.setMaxFileSize(DataSize.ofBytes(maxFileSize));
		factory.setMaxRequestSize(DataSize.ofBytes(maxRequestSize));;
		factory.setLocation(location); // String 타입이라 그냥 전달 O
		
		return factory.createMultipartConfig();
	}
	
	// MultipartResover 객체를 생성하여 Bean으로 등록
