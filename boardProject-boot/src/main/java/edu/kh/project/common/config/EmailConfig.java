...
public class EmailConfig {
	...
	// @Value : propertysource에 작성된 내용 중 key가 일치하는 value값을 얻어와 필드에 대입
	@Value("${spring.mail.username}") // org.springframework.beans.factory.annotation.Value
	private String userName;

	@Value("${spring.mail.password}")
	private String password;

	@Bean // 아래설정이 적용된 자바메일sender가 bean에 등록됨
	public JavaMailSender javaMailSender() {
		...
		return mailSender;  // 위처럼 각종 설정이 적용된 JavaMailSender를 Bean으로 등록하여 Spring 애플리케이션에서 이메일
							//을 보내기 위한 구성을 제공
