...
@Slf4j
@Component // 특정한 역할은 없으나 bean에 등록
...
public class ImageDeleteScheduling {
	...
	// cron = " 초 분 시 일 월 요일 [년도]" - 요일 : 1-7 : SUN-SAT
	
	// @Scheduled(cron="0,30 * * * * *") // 시계 초 단위가 0, 30인 경우 수행
	// @Scheduled(cron="0,15,30,45 * * * * * ") : 시계 초단위가 각 숫자인 경우 수행
	...
	public void scheduling() {
		...
		File[] boardArr = boardFolder.listFiles(); // 파일을 list형태로반환
		File[] memberArr = memberFolder.listFiles();
		
		// 두 배열을 하나로 합치기
		// imageArr 이라는 빈 배열을 boardArr과 memberArr 길이 만큼의 크기로 만듬
		File[] imageArr = new File[boardArr.length+memberArr.length];
		
		// 배열 내용 복사 ( 깊은 복사 )
		// System.arraycopy(복사할배열, 복사시작인덱스, 새로운배열, 새로운배열 대입인덱스, 복사의 범위)
