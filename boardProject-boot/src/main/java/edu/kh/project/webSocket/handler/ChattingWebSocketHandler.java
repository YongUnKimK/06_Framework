...
public class ChattingWebSocketHandler extends TextWebSocketHandler {
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		log.info("{} 종료됨 : ", session.getId());
	}
	
	// 클라이언트로부터 텍스트 메시지를 받았을 떄 실행
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		
		// message - JS에서 클라이언트로부터 전달 받은 내용
		// {"sendoerNo : "1", "targetNo: "2" , "chattingrRommNo : "2", messageContent : "ㅎㅇ"}
		
		// Jackson에서 제공하는 객체. Json에서
		...
		if(result > 0) { // DB삽입 성공
			...
			for(WebSocketSession s : sessions) {
				...
				if(loginMemberNo == msg.getTargetNo() || loginMemberNo == msg.getSenderNo()) {
					...
					// 다시 DTO(msg)를 JSON으로 변환 ( JS에 보내야 하기 떄문에 )
